struct NeuralODE{T,P<:SciMLBase.AbstractDEProblem,M<:Lux.StatefulLuxLayer,E,S} <: AbstractDEModel{T,P,M,E,S}
    ...
    error::E
    sensealg::S
    NeuralODE(problem::P, node::M, error::E, ::Type{T}=Float32; target::Int = 1, sensealg::S = InterpolatingAdjoint(; autojacvec = ReverseDiffVJP())) where {T,P,M,E,S} = 
        new{T,P,M,E,S}(problem, node, target, error, sensealg)
end
